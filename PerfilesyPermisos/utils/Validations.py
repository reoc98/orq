...
class Validations(Response):
    ...
    @classmethod
    def verifyValues(cls, data, keysRule):
        ...
        if len(notTypeKeys) > 0:
            ...
            if len(notTypeKeys) > 1:
                return cls.armadoBody(notTypeKeys, 400)
            ...
            # message = cls.auxFunction(badKeys)
        ...
        return cls.armadoBody(message, statusCode)

    # funcion que valida el envio de los campos rqueridos
    @classmethod
    def validateInput(cls, data, keysRule):
        ...
        try:
            ...
            if len(notInKeysRule) > 0:
                message = cls.auxFunction(notInKeysRule)
                ...
                message  = cls.auxFunction(notInData)
                message = f"Solo debe enviar los datos requeridos.Los campos ({message}) no deben ser enviados."

            else:
                return cls.validateInputEmpty(data, keysRule)

            return cls.armadoBody(message, 400)

        except Exception as error:
            return cls.armadoBody(str(error), 400)

    # funcion que valida que los datos no estén llegando vacios
    @classmethod
    def validateInputEmpty(cls, data, keysRule):
        try:
            ...
            if len(emptyKeys) > 0:
                message = cls.auxFunction(emptyKeys)
                ...
                return cls.verifyValues(data, keysRule)
    ...
    def json_validator(cls, data):   
        try:
            ...
            return cls.armadoBody("El formato no corresponde al requerido.", 400)